...
$app->group('/api', function () use ($app) {
  ...
  $app->get('/regates/winter',function ($request, $response, $args) {
         $sth = $this->db->prepare("SELECT * FROM regate reg INNER JOIN Challenge cha ON reg.id_challenge=cha.id_challenge WHERE cha.id_challenge = 1");
      $sth->execute();
  ...
  $app->get('/regate/summer',function ($request, $response, $args) {
         $sth = $this->db->prepare("SELECT * FROM regate reg INNER JOIN Challenge cha ON reg.id_challenge=cha.id_challenge WHERE cha.id_challenge = 2");
      $sth->execute();
	...
	//get scores by regate
	  $app->get('/resultatsbyregate/[{id}]', function ($request, $response, $args) {
         $sth = $this->db->prepare("SELECT * FROM regate reg INNER JOIN participer par ON reg.id_regate=par.id_regate INNER JOIN Voilier voi ON par.id_voilier=voi.id_voilier WHERE reg.id_regate=:id");
		 $sth->bindParam("id", $args['id']);
      $sth->execute();
      $regates = $sth->fetchAll();
      return $this->response->withJson($regates);
